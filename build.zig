...
pub fn build(b: *std.Build) void {
    ...
    const lib_mod = b.createModule(.{
        .root_source_file = b.path("src/concurrentHashMap.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const lib_unit_tests = b.addTest(.{
        .root_source_file = b.path("test/concurrentHashMap_test.zig"),
        .target = target,
        .optimize = optimize,
        .name = "concurrent-HashMap-tests",
    });

    // Add the library module as a dependency for the tests.
    lib_unit_tests.root_module.addImport("concurrent_HashMap", lib_mod);
